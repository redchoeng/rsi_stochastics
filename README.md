...
2. **매수(bullish)** — 오늘 일봉이 매수 조건을 만족하면 먼저 "오늘의 매수 후보" 1차 알림을 보내고(당일 1회), 이어서 같은 실행 안에서 오늘 15분봉(프리~애프터마켓 포함)에 같은 3가지 조건 중 하나라도 새로 나오면 별도로 진입 알림을 보낸다. 15분봉이 아직 어제 걸로 남아있으면(예: 프리마켓 시작 전) 진입 알림만 대기 — 후보 알림은 이미 나간 상태.
